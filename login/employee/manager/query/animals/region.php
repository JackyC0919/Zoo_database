...
if (isset($_POST['submit'])) {
  try {
    ...
    $sql = "SELECT *
    FROM animals
    WHERE enclosure_type_fk = :enclosure_type_fk";

    $enclosure_type_fk = $_POST['enclosure_type_fk'];

    $statement = $connection->prepare($sql);
	$statement->bindParam(':enclosure_type_fk', $enclosure_type_fk, PDO::PARAM_STR);
    $statement->execute();
...
<td><a href="animaldelete.php?id=<?php echo escape($row["animal_id"]); ?>">Delete</a></td>
  ...
  <?php } else { ?>
    > No results found for <?php echo escape($_POST['enclosure_type_fk']); ?>.
  <?php }
} ?>

<h2>Find Animal based on Region</h2>

<form method="post">
  <label for="enclosure_type_fk">Region </label>
  <input type="text" id="enclosure_type_fk" name="enclosure_type_fk">
